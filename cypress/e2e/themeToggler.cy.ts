...
describe('Theme toggler module', () => {
  it("should switch element's color within the app", () => {
    ...
    cy.get('@toggler').should('have.css', 'border-color', 'rgb(255, 138, 0)')

    // checking searchInput's colors
    cy.get('@searchInput').should('have.css', 'background-color', 'rgb(255, 255, 255)')
    cy.get('@searchInput').should('have.css', 'color', 'rgb(0, 0, 0)')

    // checking searchButton's colors
    cy.get('@searchButton').should('have.css', 'background-color', 'rgb(255, 255, 255)')
    cy.get('@searchButton').should('have.css', 'border-color', 'rgb(196, 196, 196)')

    // checking genres's colors
    cy.get('@genres').should('have.css', 'border-color', 'rgba(0, 0, 0, 0.19)')

    cy.get('[data-testid="toggler"]').first().click()

    // checking toggler's colors
    cy.get('@toggler').should('have.css', 'background-color', 'rgb(20, 20, 20)')
    cy.get('@toggler').should('have.css', 'border-color', 'rgb(255, 255, 255)')

    // checking searchInput's colors
    cy.get('@searchInput').should('have.css', 'background-color', 'rgb(20, 20, 20)')
    cy.get('@searchInput').should('have.css', 'color', 'rgb(255, 255, 255)')

    // checking searchButton's colors
    cy.get('@searchButton').should('have.css', 'background-color', 'rgb(20, 20, 20)')
    cy.get('@searchButton').should('have.css', 'border-color', 'rgb(125, 125, 125)')
  ...
  it('the theme should be saved and applied after the page is reloaded', () => {
    ...
    // checking searchButton's colors
    cy.get('[data-testid=searchButton]').should(
      'have.css',
      'background-color',
      'rgb(255, 255, 255)'
    )
